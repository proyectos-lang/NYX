...
insert into nyx.categorias (slug, nombre_es, nombre_en, imagen_portada, orden) values
...
select
  ...
  c.id, d.precio, d.tipo::nyx.tipo_producto, d.stock, d.bajo_pedido, d.orden
from (values
...
join nyx.categorias c on c.slug = d.cat_slug;

insert into nyx.producto_fotos (producto_id, url, orden, es_portada)
...
join nyx.productos p on p.sku = f.sku;

-- ---------------------------------------------------------------------------
-- Clientes y pedidos de la bandeja
-- ---------------------------------------------------------------------------

insert into nyx.clientes (nombre, email, telefono, empresa) values
...
select
  d.ref, c.id, d.estado::nyx.estado_pedido, d.metodo::nyx.metodo_entrega,
...
join nyx.clientes c on lower(c.email) = d.email;
...
join nyx.pedidos   ped  on ped.ref  = d.ref
join nyx.productos prod on prod.sku = d.sku;

insert into nyx.pedido_archivos (pedido_id, ruta_storage, nombre_archivo, bytes, mime)
...
insert into nyx.faq (pregunta_es, respuesta_es, pregunta_en, respuesta_en, orden) values
...
insert into nyx.contenido_bloques (clave, seccion, titulo, nota, orden) values
...
insert into nyx.contenido_media (bloque_id, url, tipo, orden)
select b.id, d.url, d.tipo::nyx.tipo_media, d.orden
from (values
...
insert into nyx.ajustes (clave, valor, descripcion, publico) values
  ('contacto',
   '{"email": "hola@nyx.ec", "telefono": "+1 845 972 1825", "whatsapp": "+1 845 972 1825", "ciudad": "Estados Unidos", "horario": "Lunes a viernes, 9:00 a 18:00"}'::jsonb,
